...
#---------------------------------------------------------------------
# Partitioning error as a function of t-test non-significance

file.name <- 'extended_simulation_partitioned.Rdata'
if (OVERWRITE || (! file.name %in% list.files())) {
  ...
  d_partitioned <- d_raw %>%
                      mutate(significant = ifelse(p_t <= 0.05,
                                                  'Yes', 'No')) %>%
                      group_by(label, rank, significant) %>%
                      summarize(error = median(abs(deviation)),
                                rejected = sum(p_t > 0.05) / n() * 100)

  labels <- sort(unique(as.numeric(as.character(d_partitioned$label))))
  labels <- paste(labels, '% standard deviation', sep = '')
  d_partitioned$label <- paste(as.character(d_partitioned$label), 
                               '% standard deviation', sep = '')
  d_partitioned$label <- factor(d_partitioned$label,
                                levels <- labels)


  save(d_partitioned, file=file.name)
} else {
  load(file.name)
}

# Partitioned error
p <- ggplot(d_partitioned)

p <- p + geom_bar(aes(x=rank, y=error, fill=significant),
                  stat = 'identity', position = 'identity')

p <- p + coord_cartesian(ylim=c(0, 100))

p <- p + facet_wrap(~ label, nrow=2)
p <- p + scale_fill_grey('Significant', start=0.8, end=0.2)
p <- p + ylab('Median error (%)') + xlab('Flux rank')

p <- p + theme_bw(16)
p <- p + theme(legend.position='top')

ggsave('extended_simulation_partitioned.pdf', width=7, height=5.5)
