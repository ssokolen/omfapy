...
constraints = pd.DataFrame.from_csv('./extended_constraints.csv', index_col=0)
...
unbalanced = ['ADP', 'ATP', 'mFADH', 'NADH', 'NADPH',
              'Pi', 'H2O', 'O2', 'CO2']
...
analysis = omfa.Analysis('extended_simulation.db', m, seed=1111, overwrite=True)
...
analysis.generate_samples(100, constraints['min'], constraints['max'], 
                          n_burnin=10000, n_iter=1000, xo=xo)

# Generating observations at different levels of uncertainty
for new_sd in [5, 10, 15, 20]:
    msg = 'Generating observations with {}% coefficient of variation'
    print(msg.format(new_sd))

    rel_sd = pd.Series(new_sd/100.0, sd.index)
    analysis.generate_observations(100, new_sd, rel_sd=rel_sd)

    # GLS validation
    analysis.calculate_gls(new_sd, rel_measurement=rel_sd)

    # Chi2 validation
    analysis.calculate_pi(new_sd, rel_measurement=rel_sd)
